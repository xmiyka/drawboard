...
import { hashElementsVersion, hashString } from "@drawboard/element";

import { getCommonBoundingBox } from "@drawboard/element";
...
import type { MaybePromise } from "@drawboard/common/utility-types";
...
const ALLOWED_LIBRARY_URLS = [
  ...
  "raw.githubusercontent.com/drawboard/drawboard-libraries",
];
...
const isUniqueItem = (
  ...
  return !existingLibraryItems.find((libraryItem) => {
    ...
    // detect z-index difference by checking the drawboard elements
    // are in order
    return libraryItem.elements.every((libItemDrawboardItem, idx) => {
      return (
        libItemDrawboardItem.id === targetLibraryItem.elements[idx].id &&
        libItemDrawboardItem.versionNonce ===
...
class Library {
  ...
  /** call on drawboard instance unmount */
  destroy = () => {
    this.updateQueue = [];
  ...
  // NOTE this is a high-level public API (exposed on DrawboardAPI) with
...
export const distributeLibraryItemsOnSquareGrid = (
  ...
  const resElements: DrawboardElement[] = [];
...
export const useHandleLibrary = (
  opts: {
    drawboardAPI: DrawboardImperativeAPI | null;
    /**
     * Return `true` if the library install url should be allowed.
     * If not supplied, only the drawboard.com base domain is allowed.
     */
  ...
  useEffect(() => {
    ...
    // reset on editor remount (drawboardAPI changed)
    ...
    const importLibraryFromURL = async ({
      ...
      const shouldPrompt = idToken !== drawboardAPI.id;
      ...
      try {
        await drawboardAPI.updateLibrary({
    ...
    // important this useEffect only depends on drawboardAPI so it only reruns
    // on editor remounts (the drawboardAPI changes)
    drawboardAPI,
  ]);

  // This effect is run without drawboardAPI dependency so that host apps
      ...
      const unsubOnLibraryUpdate = onLibraryUpdateEmitter.on(
        async (update, nextLibraryItems) => {
          ...
          try {
            ...
            if (isLoaded && optsRef.current.drawboardAPI) {
              optsRef.current.drawboardAPI.updateScene({
                appState: {
