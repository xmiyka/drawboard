...
import { isValidDrawboardData, isValidLibrary } from "./json";
import {
  restoreAppState,
  restoreElements,
  restoreLibraryItems,
...
export const getMimeType = (blob: Blob | string): string => {
  ...
  } else if (/\.drawboardlib$/.test(name)) {
    return MIME_TYPES.drawboardlib;
  }
  return "";
};
...
export const getFileHandleType = (handle: FileSystemHandle | null) => {
  ...
  return handle.name.match(/\.(json|drawboard|png|svg)$/)?.[1] || null;
};
 ...
 * Note: doesn't handle missing .drawboard/.drawboardlib extension  */
export const normalizeFile = async (file: File) => {
  ...
  if (file?.name?.endsWith(".drawboardlib")) {
    file = createFile(file, MIME_TYPES.drawboardlib, file.name);
  } else if (file?.name?.endsWith(".drawboard")) {
    file = createFile(file, MIME_TYPES.drawboard, file.name);
