import { isFiniteNumber, pointFrom } from "@drawboard/math";
...
import { normalizeFixedPoint } from "@drawboard/element";
import {
  updateElbowArrowPoints,
  validateElbowPoints,
} from "@drawboard/element";
import { LinearElementEditor } from "@drawboard/element";
import { bumpVersion } from "@drawboard/element";
import { getContainerElement } from "@drawboard/element";
import { detectLineHeight } from "@drawboard/element";
import {
...
import { syncInvalidIndices } from "@drawboard/element";

import { refreshTextDimensions } from "@drawboard/element";

import { getNormalizedDimensions } from "@drawboard/element";
...
import type { MarkOptional, Mutable } from "@drawboard/common/utility-types";
...
export const AllowedDrawboardActiveTools: Record<
...
const repairBinding = <T extends DrawboardArrowElement>(
  element: T,
  ...
  try {
    ...
    if (isElbowArrow(element)) {
      const fixedPointBinding:
        | DrawboardElbowArrowElement["startBinding"]
        | DrawboardElbowArrowElement["endBinding"] = {
        ...binding,
    ...
    const targetBoundElement =
      (targetElementsMap.get(binding.elementId) as DrawboardBindableElement) ||
      undefined;
    const boundElement =
      targetBoundElement ||
...
const restoreElementWithProperties = <
  T extends Required<Omit<DrawboardElement, "customData">> & {
    customData?: DrawboardElement["customData"];
    /** @deprecated */
    boundElementIds?: readonly DrawboardElement["id"][];
    /** @deprecated */
    strokeSharpness?: StrokeRoundness;
  },
  K extends Pick<T, keyof Omit<Required<T>, keyof DrawboardElement>>,
>(
  element: T,
  extra: Pick<
    T,
    ...
    keyof K
  > &
    Partial<Pick<DrawboardElement, "type" | "x" | "y" | "customData">>,
): T => {
  const base: Pick<T, keyof DrawboardElement> = {
...
export const restoreElement = (
  ...
  element: Exclude<DrawboardElement, DrawboardSelectionElement>,
  ...
  switch (element.type) {
    case "text":
      // temp fix: cleanup legacy obsidian-drawboard attribute else it'll
    ...
    case "arrow": {
      ...
      const base = {
        ...
        startBinding: repairBinding(
          element as DrawboardArrowElement,
        ...
        x,
        y,
        elbowed: (element as DrawboardArrowElement).elbowed,
        ...getSizeFromPoints(points),
      };

      // TODO: Separate arrow from linear element
      const restoredElement = isElbowArrow(element)
        ? restoreElementWithProperties(element as DrawboardElbowArrowElement, {
            ...base,
            elbowed: true,
            fixedSegments:
        ...
        : restoreElementWithProperties(element as DrawboardArrowElement, base);
...
const repairContainerElement = (
  container: Mutable<DrawboardElement>,
  elementsMap: Map<string, Mutable<DrawboardElement>>,
) => {
  if (container.boundElements) {
    ...
    const boundIds = new Set<DrawboardElement["id"]>();
...
const repairBoundElement = (
  boundElement: Mutable<DrawboardTextElement>,
  elementsMap: Map<string, Mutable<DrawboardElement>>,
) => {
...
const repairFrameMembership = (
  element: Mutable<DrawboardElement>,
  elementsMap: Map<string, Mutable<DrawboardElement>>,
) => {
  if (element.frameId) {
...
export const restoreElements = <T extends DrawboardElement>(
  ...
  opts?:
    ...
    | undefined,
): CombineBrandsIfNeeded<T, OrderedDrawboardElement> => {
  ...
  const restoredElements = syncInvalidIndices(
    (targetElements || []).reduce((elements, element) => {
      ...
      let migratedElement: DrawboardElement | null;
      try {
  ...
  for (const element of restoredElements) {
    ...
    if (isLinearElement(element)) {
      if (
        ...
        (element as Mutable<DrawboardLinearElement>).startBinding = null;
      }
      if (
        element.endBinding &&
        ...
        (element as Mutable<DrawboardLinearElement>).endBinding = null;
      }
    }
  }
  ...
  return restoredElements.map((element) => {
    ...
    return element;
  }) as CombineBrandsIfNeeded<T, OrderedDrawboardElement>;
};
 ...
 *
 * See https://github.com/drawboard/drawboard/issues/3795
 *
 ...
 * updates).
 */
export const bumpElementVersions = <T extends DrawboardElement>(
    ...
    activeTool: {
      ...updateActiveTool(
        ...
        nextAppState.activeTool.type &&
          AllowedDrawboardActiveTools[nextAppState.activeTool.type]
