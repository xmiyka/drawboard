...
import { normalizeLink, toValidURL } from "@drawboard/common";
import { hashString } from "@drawboard/element";
...
import { getBoundTextElement, getContainerElement } from "@drawboard/element";
import { getLineHeightInPx } from "@drawboard/element";
import {
  isArrowElement,
  isIframeLikeElement,
...
import { getCornerRadius, isPathALoop } from "@drawboard/element";
...
const renderElementToSvg = (
  ...
  const addToRoot = (node: SVGElement, element: DrawboardElement) => {
    if (isTestEnv()) {
  ...
  switch (element.type) {
    ...
    case "embeddable": {
      ...
      const label: DrawboardElement = createPlaceholderEmbeddableLabel(element);
      renderElementToSvg(
        label,
        elementsMap,
        rsvg,
