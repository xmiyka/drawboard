...
const TextToDiagramContent = ({
  ...
  const handleInsertMessage = async (message: TChat.ChatMessage) => {
    const mermaidContent = message.content || "";
    if (!mermaidContent.trim() || !mermaidToDrawboardLib.loaded) {
      return;
    }

    const tempDataRef = {
      current: {
        elements: [] as readonly NonDeletedDrawboardElement[],
    ...
    >
      ...
      {showPreview && (
        <TTDPreviewPanel
          ...
          error={error}
          loaded={mermaidToDrawboardLib.loaded}
...
export const TextToDiagram = ({
  ...
  return (
    <TextToDiagramContent
      mermaidToDrawboardLib={mermaidToDrawboardLib}
