...
import type { Mutable, ValueOf } from "@drawboard/common/utility-types";
import type { GlobalPoint, LocalPoint, Radians } from "@drawboard/math";

import type { DrawboardLibraryIds } from "../data/types";
...
export const DrawboardContainerContext = React.createContext<{
  container: HTMLDivElement | null;
  id: string | null;
}>({ container: null, id: null });
DrawboardContainerContext.displayName = "DrawboardContainerContext";

const DrawboardElementsContext = React.createContext<
  readonly NonDeletedDrawboardElement[]
>([]);
DrawboardElementsContext.displayName = "DrawboardElementsContext";

const DrawboardAppStateContext = React.createContext<AppState>({
  ...getDefaultAppState(),
  width: 0,
  height: 0,
  offsetLeft: 0,
...
DrawboardAppStateContext.displayName = "DrawboardAppStateContext";

const DrawboardSetAppStateContext = React.createContext<
  React.Component<any, AppState>["setState"]
>(() => {
  console.warn("Uninitialized DrawboardSetAppStateContext context!");
});
DrawboardSetAppStateContext.displayName = "DrawboardSetAppStateContext";

const DrawboardActionManagerContext = React.createContext<ActionManager>(null!);
DrawboardActionManagerContext.displayName = "DrawboardActionManagerContext";
...
export const useDrawboardContainer = () =>
  useContext(DrawboardContainerContext);
export const useDrawboardElements = () => useContext(DrawboardElementsContext);
export const useDrawboardAppState = () => useContext(DrawboardAppStateContext);
export const useDrawboardSetAppState = () =>
  useContext(DrawboardSetAppStateContext);
export const useDrawboardActionManager = () =>
  useContext(DrawboardActionManagerContext);
...
class App extends React.Component<AppProps, AppState> {
  ...
  private drawboardContainerRef = React.createRef<HTMLDivElement>();

  public scene: Scene;
  public fonts: Fonts;
  public renderer: Renderer;
  public visibleElements: readonly NonDeletedDrawboardElement[];
  ...
  public drawboardContainerValue: {
    container: HTMLDivElement | null;
    id: string;
  };
  ...
  private iFrameRefs = new Map<DrawboardElement["id"], HTMLIFrameElement>();
  /**
  ...
  private initializedEmbeds = new Set<DrawboardIframeLikeElement["id"]>();
  ...
  hitLinkElement?: NonDeletedDrawboardElement;
  ...
  constructor(props: AppProps) {
    ...
    if (drawboardAPI) {
      const api: DrawboardImperativeAPI = {
      ...
      if (typeof drawboardAPI === "function") {
        drawboardAPI(api);
      } else {
        console.error("drawboardAPI should be a function!");
      }
    }

    this.drawboardContainerValue = {
      container: this.drawboardContainerRef.current,
      id: this.id,
    };
  ...
  private onWindowMessage(event: MessageEvent) {
    ...
    switch (event.origin) {
      case "https://player.vimeo.com":
        //Allowing for multiple instances of Drawboard running in the window
        if (data.method === "paused") {
          let source: Window | null = null;
          const iframes = document.body.querySelectorAll(
            "iframe.drawboard__embeddable",
          );
          if (!iframes) {
            break;
          }
  ...
  private previousHoveredBindableElement: NonDeletedDrawboardElement | null =
    null;

  private handleDelayedBindModeChange(
    arrow: DrawboardArrowElement,
    hoveredElement: NonDeletedDrawboardElement | null,
  ) {
  ...
  private cacheEmbeddableRef(
    element: DrawboardIframeLikeElement,
  ...
  private handleEmbeddableCenterClick(element: DrawboardIframeLikeElement) {
    if (
  ...
  private isIframeLikeElementCenter(
    el: DrawboardIframeLikeElement | null,
  ...
  private updateEmbedValidationStatus = (
    element: DrawboardEmbeddableElement,
    status: boolean,
  ) => {
  ...
  private updateEmbeddables = () => {
    const iframeLikes = new Set<DrawboardIframeLikeElement["id"]>();
  ...
  private renderEmbeddables() {
    ...
    const embeddableElements = this.scene
      ...
      .filter(
        (el): el is Ordered<NonDeleted<DrawboardIframeLikeElement>> =>
      ...
      <>
        {embeddableElements.map((el) => {
          ...
          return (
            <div
              key={el.id}
              className={clsx("drawboard__embeddable-container", {
            ...
            >
              <div
                //this is a hack that addresses isse with embedded drawboard.com embeddable
                //https://github.com/drawboard/drawboard/pull/6691#issuecomment-1607383938
                /*ref={(ref) => {
                  if (!this.drawboardContainerRef.current) {
                    return;
                  }
                  const container = this.drawboardContainerRef.current;
                ...
                className="drawboard__embeddable-container__inner"
                style={{
              ...
              >
                {isHovered && (
                  <div className="drawboard__embeddable-hint">
                    {t("buttons.embeddableInteractionButton")}
                  </div>
                )}
                <div
                  className="drawboard__embeddable__outer"
                  style={{
                ...
                >
                  {(isEmbeddableElement(el)
                    ...
                    <iframe
                      ...
                      className="drawboard__embeddable"
                      srcDoc={
                        src?.type === "document"
                      ...
                      title="Drawboard Embedded Content"
  ...
  private getFrameNameDOMId = (frameElement: DrawboardElement) => {
  ...
  private resetEditingFrame = (frame: DrawboardFrameLikeElement | null) => {
    if (frame) {
  ...
  public render() {
    ...
    return (
      <div
        translate="no"
        className={clsx("drawboard drawboard-container notranslate", {
          "drawboard--view-mode":
            this.state.viewModeEnabled ||
            this.state.openDialog?.name === "elementLinkSelector",
          "drawboard--mobile": this.editorInterface.formFactor === "phone",
        })}
        style={{
        ...
        ref={this.drawboardContainerRef}
      ...
      >
        <AppContext.Provider value={this}>
          <AppPropsContext.Provider value={this.props}>
            <DrawboardContainerContext.Provider
              value={this.drawboardContainerValue}
            >
              <EditorInterfaceContext.Provider value={this.editorInterface}>
                <DrawboardSetAppStateContext.Provider value={this.setAppState}>
                  <DrawboardAppStateContext.Provider value={this.state}>
                    <DrawboardElementsContext.Provider
                      value={this.scene.getNonDeletedElements()}
                    >
                      <DrawboardActionManagerContext.Provider
                      ...
                      >
                        ...
                        <div className="drawboard-textEditorContainer" />
                        <div className="drawboard-contextMenuContainer" />
                        <div className="drawboard-eye-dropper-container" />
                        <SVGLayer
                          trails={[
                            this.laserTrails,
                            this.lassoTrail,
                        ...
                        <InteractiveCanvas
                          app={this}
                          containerRef={this.drawboardContainerRef}
                  ...
                  </DrawboardAppStateContext.Provider>
                </DrawboardSetAppStateContext.Provider>
  ...
  public focusContainer: AppClassProperties["focusContainer"] = () => {
    this.drawboardContainerRef.current?.focus();
  };
  ...
  public onInsertElements = (elements: readonly DrawboardElement[]) => {
  ...
  public onExportImage = async (
    ...
    opts: { exportingFrame: DrawboardFrameLikeElement | null },
  ) => {
  ...
  private updateMagicGeneration = ({
    ...
    frameElement: DrawboardIframeElement;
  ...
  private async onMagicFrameGenerate(
    magicFrame: DrawboardMagicFrameElement,
  ...
  private onIframeSrcCopy(element: DrawboardIframeElement) {
  ...
  public onMagicframeToolSelect = () => {
    ...
    if (selectedElements.length === 0) {
      ...
      const selectedMagicFrame: DrawboardMagicFrameElement | false =
        selectedElements.length === 1 &&
      ...
      let frame: DrawboardMagicFrameElement;
  ...
  public async componentDidMount() {
    ...
    this.drawboardContainerValue.container = this.drawboardContainerRef.current;
    ...
    if (this.props.autoFocus && this.drawboardContainerRef.current) {
      this.focusContainer();
    }

    if (supportsResizeObserver && this.drawboardContainerRef.current) {
      this.resizeObserver = new ResizeObserver(() => {
        this.refreshEditorInterface();
        this.updateDOMRect();
      });
      this.resizeObserver?.observe(this.drawboardContainerRef.current);
    }
  ...
  private addEventListeners() {
    ...
    this.onRemoveEventListenersEmitter.once(
      addEventListener(
        this.drawboardContainerRef.current,
        EVENT.WHEEL,
        this.handleWheel,
        { passive: false },
      ),
    ...
    if (this.props.detectScroll) {
      this.onRemoveEventListenersEmitter.once(
        addEventListener(
          getNearestScrollableContainer(this.drawboardContainerRef.current!),
          EVENT.SCROLL,
          this.onScroll,
          { passive: false },
        ),
  ...
  componentDidUpdate(prevProps: AppProps, prevState: AppState) {
    ...
    this.drawboardContainerRef.current?.classList.toggle(
      "theme--dark",
  ...
  private onCut = withBatchedUpdates((event: ClipboardEvent) => {
    const isDrawboardActive = this.drawboardContainerRef.current?.contains(
      document.activeElement,
    );
    if (!isDrawboardActive || isWritableElement(event.target)) {
      return;
    }
  ...
  private async insertClipboardContent(
    ...
    if (data.elements) {
      const elements = (
        data.programmaticAPI
          ? convertToDrawboardElements(
              data.elements as DrawboardElementSkeleton[],
            )
          : data.elements
      ) as readonly DrawboardElement[];
    ...
    if (!isPlainPaste && isMaybeMermaidDefinition(data.text)) {
      ...
      try {
        ...
        const elements = convertToDrawboardElements(skeletonElements, {
          regenerateIds: true,
        });
    ...
    if (
      ...
      const embeddables: NonDeleted<DrawboardEmbeddableElement>[] = [];
      for (const url of embbeddableUrls) {
        const prevEmbeddable: DrawboardEmbeddableElement | undefined =
  ...
  public pasteFromClipboard = withBatchedUpdates(
    async (event: ClipboardEvent) => {
      ...
      const isDrawboardActive =
        this.drawboardContainerRef.current?.contains(target);
      if (event && !isDrawboardActive) {
        return;
      }
        ...
        ...selectGroupsForSelectedElements(
          {
            ...
            selectedElementIds: nextElementsToSelect.reduce(
              (acc: Record<DrawboardElement["id"], true>, element) => {
  ...
  private addTextFromPaste(text: string, isPlainPaste = false) {
    ...
    const textElements = lines.reduce(
      (acc: DrawboardTextElement[], line, idx) => {
  ...
  scrollToContent = (
     ...
     * - DrawboardElement | DrawboardElement[] - element(s) objects
     */
    target:
      | string
      | DrawboardElement
      | readonly DrawboardElement[] = this.scene.getNonDeletedElements(),
    opts?: (
      | {
          fitToContent?: boolean;
          fitToViewport?: never;
    ...
    // convert provided target into DrawboardElement[] if necessary
   ...
   * */
  public addFiles: DrawboardImperativeAPI["addFiles"] = withBatchedUpdates(
    (files) => {
  ...
  public updateScene = withBatchedUpdates(
    <K extends keyof AppState>(sceneData: {
       ...
       *
       * Check [API docs](https://docs.drawboard.com/docs/@drawboard/drawboard/api/props/drawboard-api#captureUpdate) for more details.
       *
  ...
  public mutateElement = <TElement extends Mutable<DrawboardElement>>(
    element: TElement,
  ...
  public getEditorUIOffsets = (): Offsets => {
    ...
    const sidebarRect = this.drawboardContainerRef?.current
      ?.querySelector(".sidebar")
      ?.getBoundingClientRect();
    const propertiesPanelRect = this.drawboardContainerRef?.current
    ...
    (event: React.KeyboardEvent | KeyboardEvent) => {
      ...
      if (!isInputLike(event.target)) {
        ...
        if (event.key === KEYS.ESCAPE) {
          ...
          event.key === KEYS.TAB &&
          (document.activeElement === this.drawboardContainerRef?.current ||
      ...
      if (isArrowKey(event.key)) {
        ...
        selectedElements
          .filter((el): el is NonDeleted<DrawboardArrowElement> =>
            isBindingElement(el),
          )
          .filter((arrow) => {
        ...
        if (selectedElements.length === 1) {
          ...
          if (event[KEYS.CTRL_OR_CMD] || isLineElement(selectedElement)) {
            ...
            if (!isTextElement(selectedElement)) {
              container = selectedElement as DrawboardTextContainer;
            }
  ...
  private handleTextWysiwyg(
    ...
    textWysiwyg({
      ...
      element,
      drawboardContainer: this.drawboardContainerRef.current,
      app: this,
  ...
  private getTextElementAtPosition(
    x: number,
    y: number,
  ): NonDeleted<DrawboardTextElement> | null {
  ...
  private getElementAtPosition(
    ...
    opts?: (
      ...
      | {
          allHitElements: NonDeleted<DrawboardElement>[];
        }
    ) & {
      preferSelected?: boolean;
    },
  ): NonDeleted<DrawboardElement> | null {
    let allHitElements: NonDeleted<DrawboardElement>[] = [];
  ...
  private getElementsAtPosition(
  ...
  ): NonDeleted<DrawboardElement>[] {
    const iframeLikes: Ordered<DrawboardIframeElement>[] = [];
    ...
    const elements = (
      ...
      .concat(iframeLikes) as NonDeleted<DrawboardElement>[];

    return elements;
  }

  getElementHitThreshold(element: DrawboardElement) {
    return Math.max(
  ...
  private startTextEditing = ({
    ...
    container?: DrawboardTextContainer | null;
    autoEdit?: boolean;
  }) => {
    ...
    let existingTextElement: NonDeleted<DrawboardTextElement> | null = null;
  ...
  private startImageCropping = (image: DrawboardImageElement) => {
    this.store.scheduleCapture();
    this.setState({
  ...
  private handleCanvasDoubleClick = (
    ...
    if (selectedElements.length === 1 && isLinearElement(selectedElements[0])) {
      const selectedLinearElement: DrawboardLinearElement = selectedElements[0];
      if (
  ...
  private getElementLinkAtPosition = (
    ...
    hitElementMightBeLocked: NonDeletedDrawboardElement | null,
  ): DrawboardElement | undefined => {
    ...
    if (lastPointerDownHittingLinkIcon && lastPointerUpHittingLinkIcon) {
      ...
      if (url) {
        ...
        if (this.props.onLinkOpen) {
          customEvent = wrapEvent(EVENT.DRAWBOARD_LINK, event.nativeEvent);
  ...
  private getTopLayerFrameAtSceneCoords = (sceneCoords: {
    ...
    const frames = this.scene
      ...
      .filter(
        (frame): frame is DrawboardFrameLikeElement =>
  ...
  private handleCanvasPointerMove = (
    ...
    if (
      ...
      if (
        ...
        // update the state. Check https://github.com/drawboard/drawboard/pull/5508 for more details.
        flushSync(() => {
          this.setState({
    ...
    let hitElement: DrawboardElement | null = null;
    if (hitElementMightBeLocked && hitElementMightBeLocked.locked) {
      hitElement = null;
    } else {
  ...
  private handleCanvasPointerDown = (
    ...
    if (
      ...
      const element = this.state.newElement as DrawboardFreeDrawElement;
      this.updateScene({
    ...
    if (this.state.activeTool.type === "lasso") {
      ...
      if (
        ...
        this.setState((prevState) => {
          ...
          const previouslySelectedElements: DrawboardElement[] = [];
  ...
  public handleCanvasPanUsingWheelOrSpaceDrag = (
    ...
    if (!this.state.editingTextElement) {
      // necessary to prevent browser from scrolling the page if drawboard
      // not full-page #4489
      //
  ...
  private isASelectedElement(hitElement: DrawboardElement | null): boolean {
    return hitElement != null && this.state.selectedElementIds[hitElement.id];
  }

  private isHittingCommonBoundingBoxOfSelectedElements(
    point: Readonly<{ x: number; y: number }>,
    selectedElements: readonly DrawboardElement[],
  ): boolean {
  ...
  private handleTextOnPointerDown = (
    ...
    if (hasBoundTextElement(element)) {
      container = element as DrawboardTextContainer;
  ...
  private handleFreeDrawElementOnPointerDown = (
    event: React.PointerEvent<HTMLElement>,
    elementType: DrawboardFreeDrawElement["type"],
    pointerDownState: PointerDownState,
  ) => {
  ...
  private handleLinearElementOnPointerDown = (
    event: React.PointerEvent<HTMLElement>,
    elementType: DrawboardLinearElement["type"],
    pointerDownState: PointerDownState,
  ): void => {
    if (event.ctrlKey) {
      flushSync(() => {
  ...
  private createGenericElementOnPointerDown = (
    elementType: DrawboardGenericElement["type"] | "embeddable",
    pointerDownState: PointerDownState,
  ): void => {
    const [gridX, gridY] = getGridPoint(
      pointerDownState.origin.x,
  ...
  private onKeyUpFromPointerDownHandler(
    ...
    return withBatchedUpdates((event: KeyboardEvent) => {
      // Prevents focus from escaping drawboard tab
    ...
    return withBatchedUpdates((childEvent: PointerEvent) => {
      ...
      if (
        ...
        if (this.state.selectedLinearElement.elbowed) {
          ...
          if (element) {
            this.scene.mutateElement(element as DrawboardElbowArrowElement, {});
          }
        }
      ...
      if (pointerDownState.resize.isResizing) {
        ...
        const selectedFrames = this.scene
          .getSelectedElements(this.state)
          .filter((element): element is DrawboardFrameLikeElement =>
  ...
  private initializeImage = async (
    placeholderImageElement: DrawboardImageElement,
    imageFile: File,
  ) => {
    ...
    return new Promise<NonDeleted<InitializedDrawboardImageElement>>(
  ...
  private getLatestInitializedImageElement = (
    imagePlaceholder: DrawboardImageElement,
    fileId: FileId,
  ) => {
    ...
    return newElementWith(
      latestImageElement as InitializedDrawboardImageElement,
      {
        fileId,
      },
    );
  ...
  private getImageNaturalDimensions = (
    imageElement: DrawboardImageElement,
  ...
  private updateImageCache = async (
    elements: readonly InitializedDrawboardImageElement[],
    files = this.files,
  ) => {
  ...
  private addNewImagesToImageCache = async (
    imageElements: InitializedDrawboardImageElement[] = getInitializedImageElements(
      this.scene.getNonDeletedElements(),
    ),
  ...
  private clearSelection(hitElement: DrawboardElement | null): void {
    this.setState((prevState) => ({
      selectedElementIds: makeNextSelectedElementIds({}, prevState),
      activeEmbeddable: null,
      selectedGroupIds: {},
  ...
  private handleAppOnDrop = async (event: React.DragEvent<HTMLDivElement>) => {
    ...
    const drawboardLibrary_ids = dataTransferList.getData(
      MIME_TYPES.drawboardlibIds,
    );
    const drawboardLibrary_data = dataTransferList.getData(
      MIME_TYPES.drawboardlib,
    );
    if (drawboardLibrary_ids || drawboardLibrary_data) {
      try {
        let libraryItems: LibraryItems | null = null;
        if (drawboardLibrary_ids) {
          const { itemIds } = JSON.parse(
            drawboardLibrary_ids,
          ) as DrawboardLibraryIds;
        ...
        } else if (drawboardLibrary_data) {
          libraryItems = parseLibraryJSON(drawboardLibrary_data);
        }
    ...
    if (fileItems.length > 0) {
      const { file, fileHandle } = fileItems[0];
      if (file) {
        // Attempt to parse an drawboard/drawboardlib file
  ...
  loadFileToCanvas = async (
    ...
    try {
      ...
      if (ret.type === MIME_TYPES.drawboard) {
      ...
      } else if (ret.type === MIME_TYPES.drawboardlib) {
        await this.library
          .updateLibrary({
            libraryItems: file,
            merge: true,
    ...
    if (
      ...
      this.setState({
        elementsToHighlight: getElementsInResizingFrame(
          this.scene.getNonDeletedElements(),
          newElement as DrawboardFrameLikeElement,
          this.state,
          this.scene.getNonDeletedElementsMap(),
        ),
      });
  ...
  private maybeHandleResize = (
    ...
    const selectedFrames = selectedElements.filter(
      (element): element is DrawboardFrameLikeElement =>
        isFrameLikeElement(element),
    );
    ...
    if (
      ...
      const elementsToHighlight = new Set<DrawboardElement>();
      selectedFrames.forEach((frame) => {
        getElementsInResizingFrame(
          this.scene.getNonDeletedElements(),
          frame,
  ...
  private updateDOMRect = (cb?: () => void) => {
    if (this.drawboardContainerRef?.current) {
      const drawboardContainer = this.drawboardContainerRef.current;
      const {
        width,
        height,
        left: offsetLeft,
        top: offsetTop,
      } = drawboardContainer.getBoundingClientRect();
      const {
        width: currentWidth,
  ...
  private getCanvasOffsets(): Pick<AppState, "offsetTop" | "offsetLeft"> {
    if (this.drawboardContainerRef?.current) {
      const drawboardContainer = this.drawboardContainerRef.current;
      const { left, top } = drawboardContainer.getBoundingClientRect();
      return {
        offsetLeft: left,
        offsetTop: top,
      };
