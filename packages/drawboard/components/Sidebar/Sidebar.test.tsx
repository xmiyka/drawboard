...
import { DEFAULT_SIDEBAR } from "@drawboard/common";

import { Drawboard, Sidebar } from "../../index";
import {
  act,
  fireEvent,
  queryAllByTestId,
...
describe("Sidebar", () => {
  describe("General behavior", () => {
    ...
    it("should toggle sidebar using drawboardAPI.toggleSidebar()", async () => {
      const { container } = await render(
        <Drawboard>
          <Sidebar name="customSidebar">
            <div id="test-sidebar-content">42</div>
  ...
  describe("Docking behavior", () => {
    it("shouldn't be user-dockable if `onDock` not supplied", async () => {
      await assertDrawboardWithSidebar(
        <Sidebar name="customSidebar">
          <Sidebar.Header />
        </Sidebar>,
        "customSidebar",
    ...
    it("should be user-dockable when both `onDock` and `docked` supplied", async () => {
      ...
      await withDrawboardDimensions({ width: 1920, height: 1080 }, async () => {
