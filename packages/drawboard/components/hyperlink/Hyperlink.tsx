import { pointFrom, type GlobalPoint } from "@drawboard/math";
...
import { EVENT, HYPERLINK_TOOLTIP_DELAY, KEYS } from "@drawboard/common";

import { getElementAbsoluteCoords } from "@drawboard/element";

import { hitElementBoundingBox } from "@drawboard/element";

import { isElementLink } from "@drawboard/element";

import { getEmbedLink, embeddableURLValidator } from "@drawboard/element";
...
import { isEmbeddableElement } from "@drawboard/element";
...
import { useAppProps, useEditorInterface, useDrawboardAppState } from "../App";
...
import type { AppState, DrawboardProps, UIAppState } from "../../types";
...
const embeddableLinkCache = new Map<DrawboardEmbeddableElement["id"], string>();
...
export const Hyperlink = ({
  ...
  onLinkOpen: DrawboardProps["onLinkOpen"];
  setToast: (
  ...
  return (
    <div
      className="drawboard-hyperlinkContainer"
      style={{
        top: `${y}px`,
        left: `${x}px`,
        width: POPUP_WIDTH,
    ...
    >
      {isEditing ? (
        <input
          className={clsx("drawboard-hyperlinkContainer-input")}
        ...
        <a
          ...
          className="drawboard-hyperlinkContainer-link"
          target={isLocalLink(element.link) ? "_self" : "_blank"}
          onClick={(event) => {
            if (element.link && onLinkOpen) {
              const customEvent = wrapEvent(
                EVENT.DRAWBOARD_LINK,
                event.nativeEvent,
              );
              onLinkOpen(
                {
        ...
        <div className="drawboard-hyperlinkContainer-link">
          {t("labels.link.empty")}
        </div>
      )}
      <div className="drawboard-hyperlinkContainer__buttons">
        {!isEditing && (
          <ToolButton
            type="button"
            ...
            onClick={onEdit}
            className="drawboard-hyperlinkContainer--edit"
            icon={FreedrawIcon}
          />
        )}
        <ToolButton
        ...
        {linkVal && !isEmbeddableElement(element) && (
          <ToolButton
            ...
            className="drawboard-hyperlinkContainer--remove"
            icon={TrashIcon}
          />
        )}
      </div>
...
const renderTooltip = (
  ...
  tooltipDiv.classList.add("drawboard-tooltip--visible");
...
export const hideHyperlinkToolip = () => {
  ...
  if (IS_HYPERLINK_TOOLTIP_VISIBLE) {
    ...
    getTooltipDiv().classList.remove("drawboard-tooltip--visible");
  }
};
