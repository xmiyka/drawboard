...
import { actionToggleShapeSwitch } from "@drawboard/drawboard/actions/actionToggleShapeSwitch";

import { getShortcutKey } from "@drawboard/drawboard/shortcut";
...
import {
  useApp,
  useAppProps,
  useDrawboardActionManager,
  useDrawboardSetAppState,
} from "../App";
...
import type { MarkRequired } from "@drawboard/common/utility-types";
...
function CommandPaletteInner({
  ...
  const actionManager = useDrawboardActionManager();
  ...
  useEffect(() => {
    ...
    if (uiAppState && app.scene && actionManager) {
      ...
      const additionalCommands: CommandPaletteItem[] = [
        ...
        {
          label: `${t("toolBar.mermaidToDrawboard")}...`,
          category: DEFAULT_CATEGORIES.tools,
  ...
  const executeCommand = (
    ...
    if (uiAppState.openDialog?.name === "commandPalette") {
      event.stopPropagation();
      event.preventDefault();
      document.body.classList.add("drawboard-animations-disabled");
      closeCommandPalette(() => {
        command.perform({ actionManager, event });
        setLastUsed(command);

        requestAnimationFrame(() => {
          document.body.classList.remove("drawboard-animations-disabled");
        });
      });
    }
  };
