...
import { EVENT, KEYS, rgbToHex } from "@drawboard/common";
...
import { useApp, useDrawboardContainer, useDrawboardElements } from "./App";
...
export const EyeDropper: React.FC<{
  ...
  const eyeDropperContainer = useCreatePortalContainer({
    className: "drawboard-eye-dropper-backdrop",
    parentSelector: ".drawboard-eye-dropper-container",
  });
  const appState = useUIAppState();
  const elements = useDrawboardElements();
  const app = useApp();
    ...
    (event) => {
      if (
        event.target.closest(
          ".drawboard-eye-dropper-trigger, .drawboard-eye-dropper-backdrop",
        )
      ) {
        return true;
      }
  ...
  return createPortal(
    <div ref={ref} className="drawboard-eye-dropper-preview" />,
    eyeDropperContainer,
  );
};
