...
import { pointFrom, pointRotateRads, type LocalPoint } from "@drawboard/math";
...
import { wrapText } from "@drawboard/element";
...
import { ShapeCache } from "@drawboard/element";
...
type DrawboardConvertibleElement =
  | DrawboardRectangleElement
  | DrawboardDiamondElement
  | DrawboardEllipseElement
  | DrawboardLinearElement;
...
export const adjustBoundTextSize = (
  container: DrawboardTextContainer,
  boundText: DrawboardTextElementWithContainer,
  scene: Scene,
) => {
...
export const convertElementTypes = (
  ...
  if (conversionType === "generic") {
    ...
    if (nextType && isConvertibleGenericType(nextType)) {
      const convertedElements: Record<string, DrawboardElement> = {};
      ...
      for (const element of Object.values(convertedElements)) {
        ...
        if (boundText) {
          ...
          adjustBoundTextSize(
            element as DrawboardTextContainer,
            boundText,
            app.scene,
          );
        }
  ...
  if (conversionType === "linear") {
    ...
    if (isConvertibleLinearType(nextType)) {
      const convertedElements: DrawboardElement[] = [];

      const nextElementsMap: Map<DrawboardElement["id"], DrawboardElement> =
...
const isEligibleLinearElement = (element: DrawboardElement) => {
  return (
...
const toCacheKey = (
  elementId: DrawboardElement["id"],
  convertitleType: ConvertibleTypes,
) => {
  return `${elementId}:${convertitleType}` as CacheKey;
};

const filterGenericConvetibleElements = (elements: DrawboardElement[]) =>
...
const filterLinearConvertibleElements = (elements: DrawboardElement[]) =>
...
const convertLineToElbow = (line: DrawboardLinearElement): LocalPoint[] => {
...
const convertElementType = <
  TElement extends Exclude<DrawboardElement, DrawboardSelectionElement>,
>(
  element: TElement,
...
const getConvertibleType = (
  element: DrawboardConvertibleElement,
): ConvertibleTypes => {
  if (isLinearElement(element)) {
