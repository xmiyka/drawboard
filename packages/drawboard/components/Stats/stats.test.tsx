import { degreesToRadians, pointFrom, pointRotateRads } from "@drawboard/math";
import { act, fireEvent, queryByTestId } from "@testing-library/react";
import React from "react";
import { vi } from "vitest";

import { setDateTimeForTests, reseed } from "@drawboard/common";

import { isInGroup } from "@drawboard/element";

import { isTextElement } from "@drawboard/element";

import { Drawboard, getCommonBounds } from "../..";
...
describe("binding with linear elements", () => {
  ...
  it("should remain bound to linear element on small position change", async () => {
    const linear = h.elements[1] as DrawboardLinearElement;
...
describe("stats for a non-generic element", () => {
  ...
  it("text element", async () => {
    ...
    const text = h.elements[0] as DrawboardTextElement;
    mouse.clickOn(text);
