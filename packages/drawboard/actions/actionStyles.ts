...
import type { DrawboardTextElement } from "@drawboard/element/types";
...
export const actionPasteStyles = register({
  ...
  perform: (elements, appState, formData, app) => {
    ...
    if (!isDrawboardElement(pastedElement)) {
      ...
      elements: elements.map((element) => {
        if (selectedElementIds.includes(element.id)) {
          ...
          if (isTextElement(newElement)) {
            const fontSize =
              (elementStylesToCopyFrom as DrawboardTextElement).fontSize ||
              DEFAULT_FONT_SIZE;
            const fontFamily =
              (elementStylesToCopyFrom as DrawboardTextElement).fontFamily ||
              DEFAULT_FONT_FAMILY;
            newElement = newElementWith(newElement, {
              fontSize,
              fontFamily,
              textAlign:
                (elementStylesToCopyFrom as DrawboardTextElement).textAlign ||
                DEFAULT_TEXT_ALIGN,
              lineHeight:
                (elementStylesToCopyFrom as DrawboardTextElement).lineHeight ||
