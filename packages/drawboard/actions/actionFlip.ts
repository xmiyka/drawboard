import { getNonDeletedElements } from "@drawboard/element";
import { bindOrUnbindBindingElements } from "@drawboard/element";
...
import { resizeMultipleElements } from "@drawboard/element";
import { isArrowElement, isElbowArrow } from "@drawboard/element";
import { updateFrameMembershipOfSelectedElements } from "@drawboard/element";
import { CODES, KEYS, arrayToMap } from "@drawboard/common";
...
const flipElements = (
  selectedElements: NonDeleted<DrawboardElement>[],
  elementsMap: NonDeletedSceneElementsMap,
  flipDirection: "horizontal" | "vertical",
  app: AppClassProperties,
): DrawboardElement[] => {
  if (
    ...
    return selectedElements.map((element) => {
      const _element = element as DrawboardArrowElement;
  ...
  const { elbowArrows, otherElements } = selectedElements.reduce(
    (
      acc: {
        elbowArrows: DrawboardElbowArrowElement[];
        otherElements: DrawboardElement[];
      },
      element,
    ) =>
