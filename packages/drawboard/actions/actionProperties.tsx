...
import { canBecomePolygon, getNonDeletedElements } from "@drawboard/element";
...
import { hasStrokeColor } from "@drawboard/element";
...
import { deriveStylesPanelMode } from "@drawboard/common";
...
import type { CaptureUpdateActionType } from "@drawboard/element";
...
export const changeProperty = (
  ...
  appState: AppState,
  callback: (element: DrawboardElement) => DrawboardElement,
...
export const getFormValue = function <T extends Primitive>(
  ...
  getAttribute: (element: DrawboardElement) => T,
  isRelevantElement: true | ((element: DrawboardElement) => boolean),
...
const offsetElementAfterFontResize = (
  prevElement: DrawboardTextElement,
  nextElement: DrawboardTextElement,
  scene: Scene,
) => {
  ...
  return scene.mutateElement(nextElement, {
    ...
    // centering vertically is non-standard, but for Drawboard I think
    // it makes sense
...
const changeFontSize = (
  ...
  appState: AppState,
  app: AppClassProperties,
  getNewFontSize: (element: DrawboardTextElement) => number,
  fallbackValue?: DrawboardTextElement["fontSize"],
) => {
  ...
  const updatedElements = changeProperty(
    ...
    (oldElement) => {
      if (isTextElement(oldElement)) {
        ...
        let newElement: DrawboardTextElement = newElementWith(oldElement, {
...
export const actionChangeFillStyle = register<DrawboardElement["fillStyle"]>({
...
export const actionChangeSloppiness = register<DrawboardElement["roughness"]>({
...
export const actionChangeOpacity = register<DrawboardElement["opacity"]>({
  name: "changeOpacity",
...
export const actionChangeFontSize = register<DrawboardTextElement["fontSize"]>({
  name: "changeFontSize",
...
export const actionChangeFontFamily = register<{
    ...
    if (currentItemFontFamily) {
      ...
      if (selectedTextElements.length > 200) {
        ...
        while (
          ...
          const textElement = selectedTextElements[i] as DrawboardTextElement;
        ...
        elements: changeProperty(
          ...
          (oldElement) => {
            if (
              ...
              const newElement: DrawboardTextElement = newElementWith(
                oldElement,
                {
        ...
        window.document.fonts.load(fontString, chars).then((fontFaces) => {
          for (const [element, container] of elementContainerMapping) {
            ...
            if (latestElement) {
              ...
              redrawTextBoundingBox(
                latestElement as DrawboardTextElement,
                latestContainer,
                app.scene,
              );
            }
  ...
  PanelComponent: ({ elements, appState, app, updateData }) => {
    ...
    const selectedFontFamily = useMemo(() => {
      const getFontFamily = (
        elementsArray: readonly DrawboardElement[],
    ...
    return {
      elements: changeProperty(elements, appState, (el) => {
        if (isLinearElement(el)) {
          ...
          if (position === "start") {
            const element: DrawboardLinearElement = newElementWith(el, {
              startArrowhead: type,
            });
            return element;
    ...
    if (selectedId) {
      ...
      if (selected) {
        newState.selectedLinearElement = new LinearElementEditor(
          selected as DrawboardLinearElement,
          arrayToMap(elements),
        );
      }
    }
