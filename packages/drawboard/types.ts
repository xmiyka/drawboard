...
import type { MaybeTransformHandleType } from "@drawboard/element";
...
export type BinaryFiles = Record<DrawboardElement["id"], BinaryFileData>;
...
export type ElementOrToolType = DrawboardElementType | ToolType | "custom";
...
export interface AppState {
   ...
   */
  multiElement: NonDeleted<DrawboardLinearElement> | null;
  /**
   * decoupled from newElement, dragging selection only creates selectionElement
   * - set on pointer down, updated during pointer move
   */
  selectionElement: NonDeletedDrawboardElement | null;
  isBindingEnabled: boolean;
  startBoundElement: NonDeleted<DrawboardBindableElement> | null;
  suggestedBinding: NonDeleted<DrawboardBindableElement> | null;
  frameToHighlight: NonDeleted<DrawboardFrameLikeElement> | null;
  frameRendering: {
    enabled: boolean;
    name: boolean;
    outline: boolean;
  ...
  elementsToHighlight: NonDeleted<DrawboardElement>[] | null;
  /**
   * set when a new text is created or when an existing text is being edited
   */
  editingTextElement: NonDeletedDrawboardElement | null;
  activeTool: {
    /**
  ...
  currentItemFillStyle: DrawboardElement["fillStyle"];
  currentItemStrokeWidth: number;
  currentItemStrokeStyle: DrawboardElement["strokeStyle"];
  ...
  openDialog:
    ...
    | { name: "commandPalette" }
    | { name: "settings" }
    | { name: "elementLinkSelector"; sourceElementId: DrawboardElement["id"] };
  /**
  ...
  /** image cropping */
  isCropping: boolean;
  croppingElementId: DrawboardElement["id"] | null;

  /** null if no search matches found / search closed */
  searchMatches: Readonly<{
    focusedId: DrawboardElement["id"] | null;
...
export type LibraryItem_v1 = readonly NonDeleted<DrawboardElement>[];
/** @deprecated legacy: do not use outside of migration paths */
...
export type DrawboardInitialDataState = Merge<
  ImportedDataState,
  {
...
export interface DrawboardProps {
  ...
  initialData?:
    | (() => MaybePromise<DrawboardInitialDataState | null>)
    | MaybePromise<DrawboardInitialDataState | null>;
  drawboardAPI?: (api: DrawboardImperativeAPI) => void;
   ...
   */
  onDuplicate?: (
    nextElements: readonly DrawboardElement[],
    /** excludes the duplicated elements */
    prevElements: readonly DrawboardElement[],
  ) => DrawboardElement[] | void;
  renderTopLeftUI?: (
    isMobile: boolean,
    appState: UIAppState,
  ) => JSX.Element | null;
  ...
  renderEmbeddable?: (
    element: NonDeleted<DrawboardEmbeddableElement>,
    appState: AppState,
  ) => JSX.Element | null;
  aiEnabled?: boolean;
...
export type AppClassProperties = {
  ...
  drawboardContainerValue: App["drawboardContainerValue"];
...
export type PointerDownState = Readonly<{
  ...
  originalElements: Map<string, NonDeleted<DrawboardElement>>;
  resize: {
  ...
  hit: {
    ...
    // pointer down event
    element: NonDeleted<DrawboardElement> | null;
...
export interface DrawboardImperativeAPI {
...
export type FrameNameBoundsCache = {
  get: (
    frameElement: DrawboardFrameLikeElement | DrawboardMagicFrameElement,
  ) => FrameNameBounds | null;
  _cache: Map<
    string,
    FrameNameBounds & {
      zoom: AppState["zoom"]["value"];
      versionNonce: DrawboardFrameLikeElement["versionNonce"];
    }
  >;
};
...
export type EmbedsValidationStatus = Map<
  DrawboardIframeLikeElement["id"],
  boolean
>;

export type ElementsPendingErasure = Set<DrawboardElement["id"]>;

export type PendingDrawboardElements = DrawboardElement[];
...
export type GenerateDiagramToCode = (props: {
  frame: DrawboardMagicFrameElement;
  children: readonly DrawboardElement[];
