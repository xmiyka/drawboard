...
import { mutateElement } from "@drawboard/element";
...
import { getContainingFrame } from "@drawboard/element";

import { DrawboardError } from "./errors";
...
import type { ValueOf } from "@drawboard/common/utility-types";

import type { IMAGE_MIME_TYPES, STRING_MIME_TYPES } from "@drawboard/common";
import type {
  DrawboardElement,
...
type ElementsClipboard = {
  type: typeof EXPORT_DATA_TYPES.drawboardClipboard;
...
const clipboardContainsElements = (
  contents: any,
): contents is { elements: DrawboardElement[]; files?: BinaryFiles } => {
  if (
    [
      EXPORT_DATA_TYPES.drawboard,
      EXPORT_DATA_TYPES.drawboardClipboard,
      EXPORT_DATA_TYPES.drawboardClipboardWithAPI,
...
export const serializeAsClipboardJSON = ({
  ...
  const contents: ElementsClipboard = {
    type: EXPORT_DATA_TYPES.drawboardClipboard,
    ...
    for (const type of item.types) {
      ...
      try {
        if (type === MIME_TYPES.text || type === MIME_TYPES.html) {
          ...
          throw new DrawboardError(`Unsupported clipboard type: ${type}`);
        }
      } catch (error: any) {
        console.warn(
...
export const parseClipboard = async (
  ...
  try {
    ...
    const programmaticAPI =
      systemClipboardData.type === EXPORT_DATA_TYPES.drawboardClipboardWithAPI;
