...
import { getCommonBounds, getElementAbsoluteCoords } from "@drawboard/element";
...
import { newTextElement } from "@drawboard/element";
...
const truncateText = (element: DrawboardTextElement, maxWidth: number) => {
...
const addFrameLabelsAsTextElements = (
  ...
  const nextElements: NonDeletedDrawboardElement[] = [];
  for (const element of elements) {
    if (isFrameLikeElement(element)) {
      let textElement: Mutable<DrawboardTextElement> = newTextElement({
        x: element.x,
        y: element.y - FRAME_STYLE.nameOffsetY,
        fontFamily: FONT_FAMILY.Helvetica,
        fontSize: FRAME_STYLE.nameFontSize,
        lineHeight:
          FRAME_STYLE.nameLineHeight as DrawboardTextElement["lineHeight"],
...
const prepareElementsForRender = ({
  ...
  exportingFrame: DrawboardFrameLikeElement | null | undefined;
  frameRendering: AppState["frameRendering"];
  exportWithDarkMode: AppState["exportWithDarkMode"];
}) => {
  let nextElements: readonly DrawboardElement[];
...
export const encodeSvgBase64Payload = ({
  ...
  metadataElement.appendChild(
    createHTMLComment(`payload-type:${MIME_TYPES.drawboard}`),
  );
...
export const decodeSvgBase64Payload = ({ svg }: { svg: string }) => {
  if (svg.includes(`payload-type:${MIME_TYPES.drawboard}`)) {
    ...
    try {
      ...
      if (!("encoded" in encodedData)) {
        ...
        if (
          "type" in encodedData &&
          encodedData.type === EXPORT_DATA_TYPES.drawboard
        ) {
          return json;
        }
