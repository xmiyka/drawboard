...
import { ElementsDelta, AppStateDelta } from "@drawboard/element";

import { CaptureUpdateAction, StoreDelta } from "@drawboard/element";
...
describe("history", () => {
  ...
  describe("singleplayer undo/redo", () => {
    ...
    it("should create new history entry on embeddable link paste", async () => {
      await render(
        <Drawboard autoFocus={true} handleKeyboardGlobally={true} />,
      );
    ...
    describe("should support bidirectional bindings", async () => {
      let rect1: DrawboardGenericElement;
      let rect2: DrawboardGenericElement;
      let text: DrawboardTextElement;
      let arrow: DrawboardLinearElement;
      ...
      beforeEach(async () => {
        ...
        arrow = h.elements[3] as DrawboardLinearElement;
  ...
  describe("multiplayer undo/redo", () => {
    ...
    beforeEach(async () => {
      await render(
        <Drawboard handleKeyboardGlobally={true} isCollaborating={true} />,
      );
    });
    ...
    it("should override remotely added points on undo, but restore them on redo", async () => {
      ...
      API.updateScene({
        elements: [
          newElementWith(h.elements[0] as DrawboardLinearElement, {
            points: [
              pointFrom(0, 0),
              pointFrom(5, 5),
              pointFrom(10, 10),
    ...
    describe("conflicts in bound text elements and their containers", () => {
      let container: DrawboardGenericElement;
      let text: DrawboardTextElement;
      ...
      it("should rebind bindings when both are updated through the history and there no conflicting updates in the meantime", async () => {
        ...
        API.updateScene({
          elements: [
            ...
            newElementWith(h.elements[1] as DrawboardTextElement, {
      ...
      it("should preserve latest remotely added binding and unbind previous one when the text is added through history", async () => {
        ...
        API.updateScene({
          elements: [
            ...
            newElementWith(h.elements[0] as DrawboardTextElement, {
              containerId: container.id,
            }),
          ],
          captureUpdate: CaptureUpdateAction.NEVER,
        ...
        API.updateScene({
          elements: [
            ...
            h.elements[1],
            newElementWith(remoteText as DrawboardTextElement, {
              containerId: container.id,
            }),
          ],
          captureUpdate: CaptureUpdateAction.NEVER,
    ...
    describe("conflicts in arrows and their bindable elements", () => {
      ...
      it("should rebind bindings when both are updated through the history and there are no conflicting updates in the meantime", async () => {
        ...
        expect(
          (h.elements[2] as DrawboardElbowArrowElement).startBinding
            ?.fixedPoint,
        ).not.toEqual([1, 0.5001]);
        expect(
          (h.elements[2] as DrawboardElbowArrowElement).startBinding?.mode,
        ).toBe("orbit");
        expect(
          (h.elements[2] as DrawboardElbowArrowElement).endBinding,
        ).not.toEqual([1, 0.5001]);
        expect(
          (h.elements[2] as DrawboardElbowArrowElement).endBinding?.mode,
        ).toBe("orbit");
      ...
      it("should rebind bindings when both are updated through the history and the arrow got bound to a different element in the meantime", async () => {
        ...
        API.updateScene({
          elements: [
            ...
            newElementWith(h.elements[2] as DrawboardElbowArrowElement, {
              endBinding: {
      ...
      it("should rebind remotely added bindable elements when it's arrow is added through the history", async () => {
        ...
        API.updateScene({
          elements: [
            newElementWith(h.elements[0] as DrawboardElbowArrowElement, {
              startBinding: {
                elementId: rect1.id,
                fixedPoint: [0.5, 1],
                mode: "orbit",
      ...
      it("should update bound element points when rectangle was remotely moved and arrow is added back through the history", async () => {
        ...
        expect(API.getRedoStack().length).toBe(0);
        {
          ...
          const points = (h.elements[2] as DrawboardLinearElement).points[1];
          expect([
    ...
    describe("conflicts in frames and their children", () => {
      let frame: DrawboardFrameElement;
      let rect: DrawboardGenericElement;
