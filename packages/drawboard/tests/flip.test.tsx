...
import { ROUNDNESS, KEYS, arrayToMap, cloneJSON } from "@drawboard/common";

import { pointFrom, type Radians } from "@drawboard/math";

import { getBoundTextElementPosition } from "@drawboard/element";
import { getElementAbsoluteCoords } from "@drawboard/element";
import { newLinearElement } from "@drawboard/element";
...
beforeEach(async () => {
  ...
  await render(<Drawboard autoFocus={true} handleKeyboardGlobally={true} />);
  API.setAppState({
    zoom: {
...
const checkElementsBoundingBox = async (
  element1: DrawboardElement,
  element2: DrawboardElement,
...
const checkTwoPointsLineHorizontalFlip = async () => {
  const originalElement = cloneJSON(h.elements[0]) as DrawboardLinearElement;
  API.executeAction(actionFlipHorizontal);
  const newElement = h.elements[0] as DrawboardLinearElement;
  await waitFor(() => {
...
describe("image", () => {
  ...
  it("flips an unrotated image horizontally correctly", async () => {
    ...
    await createImage();
    await waitFor(() => {
      expect((h.elements[0] as DrawboardImageElement).scale).toEqual([1, 1]);
...
describe("mutliple elements", () => {
  it("with bound text flip correctly", async () => {
    ...
    const arrowText = h.elements[1] as DrawboardTextElementWithContainer;
    const arrowTextPos = getBoundTextElementPosition(
      arrow.get(),
      arrowText,
      arrayToMap(h.elements),
    )!;
    const rectText = h.elements[3] as DrawboardTextElementWithContainer;
