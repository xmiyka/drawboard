...
import { ORIG_ID, arrayToMap } from "@drawboard/common";

import { getSelectedElements } from "@drawboard/element";

import { STORAGE_KEYS } from "../../../drawboard-app/app_constants";

import { Pointer, UI } from "./helpers/ui";
import * as toolQueries from "./queries/toolQueries";

import type { AllPossibleKeys } from "@drawboard/common/utility-types";
...
const renderApp: TestRenderFn = async (ui, options) => {
  ...
  Object.defineProperty(GlobalTestState, "canvas", {
    // must be a getter because at the time of DrawboardApp render the
...
export const withDrawboardDimensions = async (
...
export const assertSelectedElements = (
  ...elements: (
    | (DrawboardElement["id"] | DrawboardElement)[]
    | DrawboardElement["id"]
    | DrawboardElement
  )[]
) => {
...
export const getCloneByOrigId = <T extends boolean = false>(
  origId: DrawboardElement["id"],
  returnNullIfNotExists: T = false as T,
): T extends true ? DrawboardElement | null : DrawboardElement => {
  ...
  return null as T extends true ? DrawboardElement | null : DrawboardElement;
};
 ...
 */
export const assertElements = <T extends AllPossibleKeys<DrawboardElement>>(
  actualElements: readonly DrawboardElement[],
  /** array order matters */
  ...
  const selectedElementIds = expectedElementsWithIds.reduce(
    (acc: Record<DrawboardElement["id"], true>, el) => {
      if (el.selected) {
        acc[el.id] = true;
      }
      return acc;
