...
import { pointFrom, type LocalPoint, type Radians } from "@drawboard/math";

import { DEFAULT_VERTICAL_ALIGN, ROUNDNESS, assertNever } from "@drawboard/common";
...
import { isLinearElementType } from "@drawboard/element";
import { getSelectedElements } from "@drawboard/element";
import { selectGroupsForSelectedElements } from "@drawboard/element";

import { FONT_SIZES } from "@drawboard/common";
...
export class API {
  ...
  static setElements = (elements: readonly DrawboardElement[]) => {
    act(() => {
      h.elements = elements;
    });
  };

  static setSelectedElements = (elements: DrawboardElement[], editingGroupId?: string | null) => {
    act(() => {
      h.setState({
        ...selectGroupsForSelectedElements(
        {
          ...
          selectedElementIds: elements.reduce((acc, element) => {
            acc[element.id] = true;
            return acc;
          }, {} as Record<DrawboardElement["id"], true>),
        },
        elements,
        h.state,
        h.app,
  ...
  static updateElement = <T extends DrawboardElement>(
  ...
  static getSelectedElement = (): DrawboardElement => {
  ...
  static getElement = <T extends DrawboardElement>(element: T): T => {
    return h.app.scene.getElementsMapIncludingDeleted().get(element.id) as T || element;
  }

  static createElement = <
    T extends Exclude<DrawboardElementType, "selection"> = "rectangle",
  >({
    // @ts-ignore
    type = "rectangle",
    id,
    ...
    id?: string;
    isDeleted?: boolean;
    frameId?: DrawboardElement["id"] | null;
    index?: DrawboardElement["index"];
    groupIds?: DrawboardElement["groupIds"];
    // generic element props
    strokeColor?: DrawboardGenericElement["strokeColor"];
    backgroundColor?: DrawboardGenericElement["backgroundColor"];
    fillStyle?: DrawboardGenericElement["fillStyle"];
    strokeWidth?: DrawboardGenericElement["strokeWidth"];
    strokeStyle?: DrawboardGenericElement["strokeStyle"];
    roundness?: DrawboardGenericElement["roundness"];
    roughness?: DrawboardGenericElement["roughness"];
    opacity?: DrawboardGenericElement["opacity"];
    // text props
    text?: T extends "text" ? DrawboardTextElement["text"] : never;
    fontSize?: T extends "text" ? DrawboardTextElement["fontSize"] : never;
    fontFamily?: T extends "text" ? DrawboardTextElement["fontFamily"] : never;
    textAlign?: T extends "text" ? DrawboardTextElement["textAlign"] : never;
    verticalAlign?: T extends "text"
      ? DrawboardTextElement["verticalAlign"]
      : never;
    boundElements?: DrawboardGenericElement["boundElements"];
    containerId?: T extends "text"
      ? DrawboardTextElement["containerId"]
      : never;
    points?: T extends "arrow" | "line" | "freedraw" ? readonly LocalPoint[] : never;
    locked?: boolean;
    fileId?: T extends "image" ? string : never;
    scale?: T extends "image" ? DrawboardImageElement["scale"] : never;
    status?: T extends "image" ? DrawboardImageElement["status"] : never;
    startBinding?: T extends "arrow"
      ? DrawboardArrowElement["startBinding"] | DrawboardElbowArrowElement["startBinding"]
      : never;
    endBinding?: T extends "arrow"
      ? DrawboardArrowElement["endBinding"] | DrawboardElbowArrowElement["endBinding"]
      : never;
    startArrowhead?: T extends "arrow"
      ? DrawboardArrowElement["startArrowhead"] | DrawboardElbowArrowElement["startArrowhead"]
      : never;
    endArrowhead?: T extends "arrow"
      ? DrawboardArrowElement["endArrowhead"] | DrawboardElbowArrowElement["endArrowhead"]
      : never;
    elbowed?: boolean;
    ...
    ? DrawboardMagicFrameElement
    : DrawboardGenericElement => {
    let element: Mutable<DrawboardElement> = null!;
