import { pointFrom, pointRotateRads } from "@drawboard/math";
...
import { cropElement } from "@drawboard/element";
import {
  getTransformHandles,
...
import { KEYS, arrayToMap } from "@drawboard/common";
...
import type { TransformHandleType } from "@drawboard/element";
import type {
  DrawboardElement,
...
export class Pointer {
  ...
  select(
    ...
    elements: DrawboardElement | DrawboardElement[],
  ) {
    API.clearSelection();
  ...
  clickOn(element: DrawboardElement) {
    this.reset();
    this.click(
      ...getElementPointForSelection(
        element,
  ...
  doubleClickOn(element: DrawboardElement) {
    this.reset();
    this.doubleClick(
      ...getElementPointForSelection(
        element,
...
const transform = (
  element: DrawboardElement | DrawboardElement[],
...
const proxy = <T extends DrawboardElement>(
  element: T,
): typeof element & {
      ...
      get(target, prop) {
        ...
        if (prop === "get") {
          if (currentElement.hasOwnProperty("get")) {
            throw new Error(
              "trying to get `get` test property, but DrawboardElement seems to define its own",
            );
          }
...
export class UI {
  ...
  /**
   * Creates an Drawboard element, and returns a proxy that wraps it so that
  ...
  static async editText<
    T extends DrawboardTextElement | DrawboardTextContainer,
  ...
  static group(elements: DrawboardElement[]) {
  ...
  static ungroup(elements: DrawboardElement[]) {
    mouse.select(elements);
    Keyboard.withModifierKeys({ ctrl: true, shift: true }, () => {
      Keyboard.keyPress(KEYS.G);
    });
