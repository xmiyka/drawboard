...
import { DEFAULT_SIDEBAR, FONT_FAMILY, ROUNDNESS } from "@drawboard/common";

import { newElementWith } from "@drawboard/element";
import * as sizeHelpers from "@drawboard/element";
...
import type { NormalizedZoomValue } from "@drawboard/drawboard/types";
...
describe("restoreElements", () => {
  ...
  it("should not call isInvisiblySmallElement when element is a selection element", () => {
    const selectionEl = { type: "selection" } as DrawboardElement;
  ...
  it("should restore line and draw elements correctly", () => {
    ...
    const restoredLine = restoredElements[0] as DrawboardLinearElement;
    const restoredDraw = restoredElements[1] as DrawboardLinearElement;
  ...
  it("should restore arrow element correctly", () => {
    ...
    const restoredArrow = restoredElements[0] as DrawboardLinearElement;
  ...
  it("when the number of points of a line is greater or equal 2", () => {
    ...
    const restoredLine_0 = restoredElements[0] as DrawboardLinearElement;
    const restoredLine_1 = restoredElements[1] as DrawboardLinearElement;
...
describe("restoreAppState", () => {
  ...
  it("when imported data state has a not allowed Drawboard Element Types", () => {
    const stubImportedAppState: any = getDefaultAppState();

    stubImportedAppState.activeTool = "not allowed Drawboard Element Types";
    const stubLocalAppState = getDefaultAppState();
