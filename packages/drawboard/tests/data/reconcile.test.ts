...
import { randomInteger, cloneJSON } from "@drawboard/common";
...
import type { RemoteDrawboardElement } from "../../data/reconcile";
...
type Cache = Record<string, DrawboardElement | undefined>;
...
const test = <U extends `${string}:${"L" | "R"}`>(
  ...
  const reconciled = reconcileElements(
    cloneJSON(_local),
    cloneJSON(_remote) as RemoteDrawboardElement[],
    {} as AppState,
  );
  ...
  try {
    assert.deepEqual(
      reconcileElements(
        cloneJSON(_remote),
        cloneJSON(_local as RemoteDrawboardElement[]),
        {} as AppState,
      ).map((x) => x.id),
      reconciledIds,
  ...
  try {
    assert.deepEqual(
      reconcileElements(
        cloneJSON(_remote),
        cloneJSON(reconciled as unknown as RemoteDrawboardElement[]),
        {} as AppState,
      ).map((x) => x.id),
      reconciledIds,
  ...
  it("test identical elements reconciliation", () => {
    const testIdentical = (
      ...
      const ret = reconcileElements(
        local as unknown as OrderedDrawboardElement[],
        remote as unknown as RemoteDrawboardElement[],
        {} as AppState,
      );
