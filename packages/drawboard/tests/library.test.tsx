...
import { MIME_TYPES, ORIG_ID } from "@drawboard/common";
...
import type { DrawboardGenericElement } from "@drawboard/element/types";
...
const libraryJSONPromise = API.readFile(
  "./fixtures/fixture_library.drawboardlib",
  "utf8",
);
...
const mockLibraryFilePromise = new Promise<Blob>(async (resolve, reject) => {
  try {
    resolve(
      new Blob([await libraryJSONPromise], { type: MIME_TYPES.drawboardlib }),
    );
  } catch (error) {
    reject(error);
  }
...
describe("library items inserting", () => {
  beforeEach(async () => {
    ...
    await render(<Drawboard initialData={{ libraryItems }} />);
  });
    ...
    await API.drop([
      {
        ...
        type: MIME_TYPES.drawboardlibIds,
      },
    ]);
    ...
    await API.drop([
      {
        ...
        file: await API.loadFile("./fixtures/fixture_library.drawboardlib"),
      },
    ]);
    await waitFor(async () => {
      expect(await h.app.library.getLatestLibrary()).toEqual([
...
describe("distributeLibraryItemsOnSquareGrid()", () => {
  it("should distribute items on a grid", async () => {
    const createLibraryItem = (
      elements: DrawboardGenericElement[],
    ): LibraryItem => {
      return {
        id: `id-${Date.now()}`,
        elements,
