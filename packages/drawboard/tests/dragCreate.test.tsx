...
import { KEYS, reseed } from "@drawboard/common";
...
import type { DrawboardLinearElement } from "@drawboard/element/types";
...
describe("Test dragCreate", () => {
  describe("add element to the scene when pointer dragging long enough", () => {
    it("rectangle", async () => {
      const { getByToolName, container } = await render(<Drawboard />);
      // select tool
    ...
    it("arrow", async () => {
      ...
      const element = h.elements[0] as DrawboardLinearElement;
  ...
  describe("do not add element to the scene if size is too small", () => {
    ...
    it("arrow", async () => {
      const { getByToolName, container } = await render(
        <Drawboard handleKeyboardGlobally={true} />,
      );
      // select tool
      const tool = getByToolName("arrow");
      fireEvent.click(tool);
