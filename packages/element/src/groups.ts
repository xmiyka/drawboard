...
export const selectGroupsForSelectedElements = (function () {
  ...
  let lastSelectedElements: readonly NonDeleted<DrawboardElement>[] | null =
    null;
  let lastElements: readonly NonDeleted<DrawboardElement>[] | null = null;
  let lastReturnValue: SelectGroupsReturnType | null = null;

  const _selectGroups = (
    selectedElements: readonly NonDeleted<DrawboardElement>[],
    elements: readonly NonDeleted<DrawboardElement>[],
...
export const editGroupForSelectedElement = (
  appState: AppState,
  element: NonDeleted<DrawboardElement>,
): AppState => {
  return {
    ...appState,
...
export const isElementInGroup = (element: DrawboardElement, groupId: string) =>
...
export const getElementsInGroup = (
  ...
  const elementsInGroup: DrawboardElement[] = [];
...
export const addToGroup = (
  prevGroupIds: DrawboardElement["groupIds"],
  newGroupId: GroupId,
  editingGroupId: AppState["editingGroupId"],
) => {
...
export const getMaximumGroups = (
  ...
  const groups: Map<String, DrawboardElement[]> = new Map<
    String,
    DrawboardElement[]
  >();
  elements.forEach((element: DrawboardElement) => {
    const groupId =
...
export const isInGroup = (element: NonDeletedDrawboardElement) => {
...
export const getSelectedElementsByGroup = (
  ...
  const groups: Map<string, DrawboardElement[]> = new Map();
  const elements: Map<string, DrawboardElement[]> = new Map();

  // helper function to add an element to the elements map
  const addToElementsMap = (element: DrawboardElement) => {
    // elements
  ...
  const addToGroupsMap = (element: DrawboardElement, groupId: string) => {
    // groups
    const currentGroupMembers = groups.get(groupId) || [];
    const boundTextElement = getBoundTextElement(element, elementsMap);
