 ...
 */
export const duplicateElement = <TElement extends DrawboardElement>(
...
export const duplicateElements = (
  opts: {
    ...
    randomizeSeed?: boolean;
    overrides?: (data: {
      duplicateElement: DrawboardElement;
      origElement: DrawboardElement;
      origIdToDuplicateId: Map<DrawboardElement["id"], DrawboardElement["id"]>;
    }) => Partial<DrawboardElement>;
  } & (
    | {
        /**
    ...
    | {
         ...
         */
        type: "in-place";
        idsOfElementsToDuplicate: Map<DrawboardElement["id"], DrawboardElement>;
        appState: {
          editingGroupId: AppState["editingGroupId"];
  ...
  // loop over them.
  const processedIds = new Map<DrawboardElement["id"], true>();
  const groupIdMap = new Map();
  const duplicatedElements: DrawboardElement[] = [];
  const origElements: DrawboardElement[] = [];
  ...
  const duplicateIdToOrigElement = new Map<
    ...
    DrawboardElement
  >();
  const duplicateElementsMap = new Map<string, DrawboardElement>();
  ...
  const elementsWithDuplicates: DrawboardElement[] = elements.slice();
  ...
  const copyElements = <T extends DrawboardElement | DrawboardElement[]>(
    element: T,
  ): T extends DrawboardElement[]
    ? DrawboardElement[]
    : DrawboardElement | null => {
    const elements = castArray(element);

    const _newElements = elements.reduce((acc: DrawboardElement[], element) => {
  ...
  const insertBeforeOrAfterIndex = (
    index: number,
    elements: DrawboardElement | null | DrawboardElement[],
  ) => {
    if (!elements) {
      return;
    }
...
// Simplified deep clone for the purpose of cloning DrawboardElement.
//
...
// passing DrawboardElement as the top-level argument).
const _deepCopyElement = (val: any, depth: number = 0) => {
  ...
  // don't support them on drawboard elements yet. If we do, we need to make
...
/**
 * Clones DrawboardElement data structure. Does not regenerate id, nonce, or
 ...
 * Typed arrays and other non-null objects.
 */
export const deepCopyElement = <T extends DrawboardElement>(
  val: T,
): Mutable<T> => {
