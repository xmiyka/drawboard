...
import { isNonDeletedElement } from "@drawboard/element";
import { isFrameLikeElement } from "@drawboard/element";
import { getElementsInGroup } from "@drawboard/element";
...
import { mutateElement, type ElementUpdate } from "@drawboard/element";
...
import type { AppState } from "../../drawboard/types";
...
const getNonDeletedElements = <T extends DrawboardElement>(
  ...
  for (const element of allElements) {
    if (!element.isDeleted) {
      ...
      elementsMap.set(
        element.id,
        element as Ordered<NonDeletedDrawboardElement>,
      );
    }
  }
...
// in our codebase
export type DrawboardElementsIncludingDeleted = readonly DrawboardElement[];
...
export class Scene {
  ...
  private nonDeletedElements: readonly Ordered<NonDeletedDrawboardElement>[] =
    [];
  ...
  private elements: readonly OrderedDrawboardElement[] = [];
  private nonDeletedFramesLikes: readonly NonDeleted<DrawboardFrameLikeElement>[] =
    [];
  private frames: readonly DrawboardFrameLikeElement[] = [];
  private elementsMap = toBrandedType<SceneElementsMap>(new Map());
  private selectedElementsCache: {
    selectedElementIds: AppState["selectedElementIds"] | null;
    elements: readonly NonDeletedDrawboardElement[] | null;
    cache: Map<SelectionHash, NonDeletedDrawboardElement[]>;
  } = {
  ...
  getNonDeletedFramesLikes(): readonly NonDeleted<DrawboardFrameLikeElement>[] {
    return this.nonDeletedFramesLikes;
  }

  getElement<T extends DrawboardElement>(id: T["id"]): T | null {
    return (this.elementsMap.get(id) as T | undefined) || null;
  }

  getNonDeletedElement(
    id: DrawboardElement["id"],
   ...
   *
   * Maps all current drawboard elements, invoking the callback for each
   ...
   */
  mapElements(
    iteratee: (element: DrawboardElement) => DrawboardElement,
  ): boolean {
    let didChange = false;
  ...
  replaceAllElements(
    ...
    const nextFrameLikes: DrawboardFrameLikeElement[] = [];
  ...
  insertElementAtIndex(element: DrawboardElement, index: number) {
  ...
  insertElementsAtIndex(elements: DrawboardElement[], index: number) {
    if (!elements.length) {
      return;
    }
  ...
  insertElement = (element: DrawboardElement) => {
  ...
  insertElements = (elements: DrawboardElement[]) => {
    if (!elements.length) {
      return;
    }
  ...
  getContainerElement = (
    element:
      | (DrawboardElement & {
          containerId: DrawboardElement["id"] | null;
        })
      | null,
  ) => {
    if (!element) {
  ...
  getElementsFromId = (id: string): DrawboardElement[] => {
  ...
  mutateElement<TElement extends Mutable<DrawboardElement>>(
    element: TElement,
