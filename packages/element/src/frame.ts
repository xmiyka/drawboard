...
import { isPointWithinBounds, pointFrom } from "@drawboard/math";
import { doLineSegmentsIntersect } from "@drawboard/utils/bbox";
import { elementsOverlappingBBox } from "@drawboard/utils/withinBounds";
...
import type { ReadonlySetLike } from "@drawboard/common/utility-types";
...
import type { DrawboardElementsIncludingDeleted } from "./Scene";
...
export const bindElementsToFramesAfterDuplication = (
  ...
  origElements: readonly DrawboardElement[],
...
export function isElementIntersectingFrame(
  ...
  frame: DrawboardFrameLikeElement,
...
export const isCursorInFrame = (
  ...
  frame: NonDeleted<DrawboardFrameLikeElement>,
  elementsMap: ElementsMap,
) => {
 ...
 */
export const groupByFrameLikes = (elements: readonly DrawboardElement[]) => {
...
export const getFrameChildren = (
  ...
  const frameChildren: DrawboardElement[] = [];
...
export const getFrameLikeElements = (
  allElements: DrawboardElementsIncludingDeleted,
): DrawboardFrameLikeElement[] => {
  return allElements.filter((element): element is DrawboardFrameLikeElement =>
    isFrameLikeElement(element),
  );
};

/**
 * Returns DrawboardFrameElements and non-frame-children elements.
 *
...
export const getElementsInResizingFrame = (
  ...
  const nextElementsInFrame = new Set<DrawboardElement>(prevElementsInFrame);
...
export const getElementsInNewFrame = (
  elements: DrawboardElementsIncludingDeleted,
  frame: DrawboardFrameLikeElement,
  elementsMap: ElementsMap,
) => {
...
export const getContainingFrame = (
  ...
  return (elementsMap.get(element.frameId) ||
    null) as null | DrawboardFrameLikeElement;
};
...
export const filterElementsEligibleAsFrameChildren = (
  ...
  const otherFrames = new Set<DrawboardFrameLikeElement["id"]>();
  const elementsMap = arrayToMap(elements);
  ...
  const processedGroups = new Set<DrawboardElement["id"]>();

  const eligibleElements: DrawboardElement[] = [];
...
export const addElementsToFrame = <T extends ElementsMapOrArray>(
  allElements: T,
  elementsToAdd: NonDeletedDrawboardElement[],
  frame: DrawboardFrameLikeElement,
  appState: AppState,
): T => {
  const elementsMap = arrayToMap(allElements);
  const currTargetFrameChildrenMap = new Map<DrawboardElement["id"], true>();
  for (const element of allElements.values()) {
  ...
  const finalElementsToAdd: DrawboardElement[] = [];
...
export const removeElementsFromFrame = (
  elementsToRemove: ReadonlySetLike<NonDeletedDrawboardElement>,
  elementsMap: ElementsMap,
) => {
  const _elementsToRemove = new Map<DrawboardElement["id"], DrawboardElement>();

  const toRemoveElementsByFrame = new Map<
    DrawboardFrameLikeElement["id"],
    DrawboardElement[]
  >();
...
export const removeAllElementsFromFrame = <T extends DrawboardElement>(
...
export const replaceAllElementsInFrame = <T extends DrawboardElement>(
  allElements: readonly T[],
  nextElementsInFrame: DrawboardElement[],
  frame: DrawboardFrameLikeElement,
...
>(
  ...
  const elementsToFilter = new Set<DrawboardElement>(selectedElements);
  ...
  const elementsToRemove = new Set<DrawboardElement>();
...
export const isElementInFrame = (
  ...
  appState: StaticCanvasAppState,
  opts?: {
    targetFrame?: DrawboardFrameLikeElement;
...
export const getDefaultFrameName = (element: DrawboardFrameLikeElement) => {
  // TODO name frames "AI" only if specific to AI frames
  return isFrameElement(element) ? DEFAULT_FRAME_NAME : DEFAULT_AI_FRAME_NAME;
};

export const getFrameLikeTitle = (element: DrawboardFrameLikeElement) => {
