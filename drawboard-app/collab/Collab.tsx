...
import { APP_NAME, cloneJSON, EVENT, toBrandedType } from "@drawboard/common";
import {
  IDLE_THRESHOLD,
  ACTIVE_THRESHOLD,
  UserIdleState,
...
import { decryptData } from "@drawboard/drawboard/data/encryption";
import { getVisibleSceneBounds } from "@drawboard/element";
import { newElementWith } from "@drawboard/element";
import { isImageElement, isInitializedImageElement } from "@drawboard/element";
import { AbortError } from "@drawboard/drawboard/errors";
import { t } from "@drawboard/drawboard/i18n";
import { withBatchedUpdates } from "@drawboard/drawboard/reactUtils";

import throttle from "lodash.throttle";
import { PureComponent } from "react";

import { bumpElementVersions } from "@drawboard/drawboard/data/restore";
...
import type {
  ...
  RemoteDrawboardElement,
} from "@drawboard/drawboard/data/reconcile";

import type { SocketUpdateDataSource, SyncableDrawboardElement } from "../data";
...
class Collab extends PureComponent<CollabProps, CollabState> {
  ...
  drawboardAPI: CollabProps["drawboardAPI"];
  ...
  constructor(props: CollabProps) {
    ...
    this.drawboardAPI = props.drawboardAPI;
  ...
  componentDidMount() {
    ...
    const unsubOnUserFollow = this.drawboardAPI.onUserFollow((payload) => {
      this.portal.socket && this.portal.broadcastUserFollowed(payload);
    });
    const throttledRelayUserViewportBounds = throttleRAF(
      this.relayVisibleSceneBounds,
    );
    const unsubOnScrollChange = this.drawboardAPI.onScrollChange(() =>
  ...
  saveCollabRoomToFirebase = async (
    syncableElements: readonly SyncableDrawboardElement[],
  ) => {
  ...
  stopCollaboration = (keepRemoteState = true) => {
    ...
    this.saveCollabRoomToFirebase(
      getSyncableElements(this.drawboardAPI.getSceneElementsIncludingDeleted()),
    );
    ...
    if (!keepRemoteState) {
      ...
      const elements = this.drawboardAPI
  ...
  private fetchImageFilesFromFirebase = async (opts: {
    ...
    const unfetchedImages = opts.elements
      ...
      .map((element) => (element as InitializedDrawboardImageElement).fileId);
  ...
  startCollaboration = async (
    ...
    const scenePromise = resolvablePromise<
      | (ImportedDataState & { elements: readonly OrderedDrawboardElement[] })
      | null
    >();
    ...
    if (existingRoomLinkData) {
      ...
      this.drawboardAPI.resetScene();
    } else {
      const elements = this.drawboardAPI.getSceneElements().map((element) => {
        if (isImageElement(element) && element.status === "saved") {
          return newElementWith(element, { status: "pending" });
        }
        return element;
      ...
      async (encryptedData: ArrayBuffer, iv: Uint8Array<ArrayBuffer>) => {
        ...
        switch (decryptedData.type) {
          ...
          case WS_SUBTYPES.INIT: {
            if (!this.portal.socketInitialized) {
              ...
              const remoteElements = toBrandedType<
                readonly RemoteDrawboardElement[]
          ...
          case WS_SUBTYPES.UPDATE:
            this.handleRemoteSceneUpdate(
              this._reconcileElements(
                toBrandedType<readonly RemoteDrawboardElement[]>(
                  decryptedData.payload.elements,
                ),
              ),
            );
          ...
          case WS_SUBTYPES.USER_VISIBLE_SCENE_BOUNDS: {
            ...
            const appState = this.drawboardAPI.getAppState();
  ...
  private loadImageFiles = throttle(async () => {
    const { loadedFiles, erroredFiles } =
      await this.fetchImageFilesFromFirebase({
        elements: this.drawboardAPI.getSceneElementsIncludingDeleted(),
      });

    this.drawboardAPI.addFiles(loadedFiles);

    updateStaleImageStatuses({
      drawboardAPI: this.drawboardAPI,
      erroredFiles,
      elements: this.drawboardAPI.getSceneElementsIncludingDeleted(),
    });
  }, LOAD_IMAGES_TIMEOUT);
  ...
  private handleRemoteSceneUpdate = (
    elements: ReconciledDrawboardElement[],
  ) => {
    this.drawboardAPI.updateScene({
      elements,
      captureUpdate: CaptureUpdateAction.NEVER,
  ...
  setCollaborators(sockets: SocketId[]) {
    ...
    this.drawboardAPI.updateScene({ collaborators });
  }
  ...
  public getSceneElementsIncludingDeleted = () => {
    return this.drawboardAPI.getSceneElementsIncludingDeleted();
  };
  ...
  broadcastElements = (elements: readonly OrderedDrawboardElement[]) => {
    if (
      getSceneVersion(elements) >
  ...
  syncElements = (elements: readonly OrderedDrawboardElement[]) => {
  ...
  queueBroadcastAllElements = throttle(() => {
    this.portal.broadcastScene(
      WS_SUBTYPES.UPDATE,
      this.drawboardAPI.getSceneElementsIncludingDeleted(),
      true,
    );
